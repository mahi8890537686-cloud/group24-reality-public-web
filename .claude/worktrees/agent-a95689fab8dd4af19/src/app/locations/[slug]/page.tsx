...
export default async function LocationDetailPage({ params }: PageProps) {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <nav aria-label="Breadcrumb" className="mb-8">
            <ol className="flex items-center gap-2 text-sm font-inter text-white/50">
              ...
              <li><Link href="/locations" className="hover:text-gold-400 transition-colors">Locations</Link></li>
              <li aria-hidden="true">/</li>
      ...
      <div className="bg-sand-50 border-t border-sand-100 py-16 sm:py-24" aria-labelledby="location-faq-heading">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="max-w-3xl mx-auto space-y-3">
            {faqs.map((faq) => (
              <details
                key={faq.question}
                className="group border border-sand-200 rounded-xl overflow-hidden bg-white [&_summary::-webkit-details-marker]:hidden"
              >
                <summary className="list-none w-full flex items-start justify-between gap-4 px-6 py-5 text-left cursor-pointer hover:bg-sand-50 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-gold-400">
                  <span className="font-playfair font-semibold text-navy-950 text-base leading-snug">
                    {faq.question}
                  </span>
                  <ChevronDown
                    className="w-5 h-5 text-gold-500 shrink-0 mt-0.5 transition-transform duration-300 group-open:rotate-180"
                    aria-hidden="true"
                  />
                </summary>
      ...
      {totalAvailable > 0 && (
        <div className="bg-sand-50 border-t border-sand-100 py-12">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="font-inter text-slate-600 text-sm">
              <span className="font-semibold text-navy-950">{totalAvailable}</span> verified{' '}
            ...
            <Link
              ...
              className="group inline-flex items-center gap-2 bg-navy-950 hover:bg-gold-400 text-white hover:text-navy-950 font-inter font-bold px-8 py-4 rounded-xl transition-all duration-200 hover:shadow-xl hover:shadow-gold-400/25 active:scale-[0.97] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400 whitespace-nowrap"
            >
