  ...
  return (
    <div className="space-y-8">
      ...
      {highlights.length > 0 && (
        <div>
          <h2 className="font-playfair font-semibold text-navy-950 text-xl mb-4 flex items-center gap-2">
            <Zap className="w-5 h-5 text-gold-400" aria-hidden="true" />
            Key Highlights
          </h2>
      ...
      {amenities.length > 0 && (
        <div>
          <h2 className="font-playfair font-semibold text-navy-950 text-xl mb-4 flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-gold-400" aria-hidden="true" />
            Amenities & Features
          </h2>
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {amenities.map((amenity) => (
              <li
                key={amenity}
                className="flex items-center gap-2.5 text-sm font-inter text-slate-700 bg-sand-50 border border-sand-100 rounded-xl px-4 py-3"
              >
